...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  while (vma != NULL)
  {
    if ((vma->vm_id != vmaid) && !(vmastart > vma->vm_end || vmaend < vma->vm_start))
      return -1;
    vma = vma->vm_next;
  }
  return 0; // no overlap
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz)
{
  ...
  struct vm_rg_struct *area = malloc(sizeof(struct vm_rg_struct)); // this area is used for checking overlap
  struct vm_area_struct *cur_vma = get_vma_by_num(caller->mm, vmaid);
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  while (vma != NULL)
  {
    if ((vma->vm_id != vmaid) && !(vmastart >= vma->vm_end || vmaend <= vma->vm_start)) // because vm_end is 1 byte higher than the mapped vm area
      return -1;
    vma = vma->vm_next;
  }
  return 0; // no overlap
